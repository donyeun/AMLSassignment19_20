...
# sys.stdout = open(cfg['task_b']['b1']['log_path'],"w") # for logging purposes

b1 = TaskB1(cfg, dp, cl)
X, Y = b1.feature_extraction()
X_train, X_test, Y_train, Y_test = b1.train_test_split(X, Y)
print(len(X_train))
clf, acc_b1_train, acc_b1_val = b1.train(X_train, Y_train)
acc_b1_test = b1.test(X_test, Y_test, clf)
print(acc_b1_test)
